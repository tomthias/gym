...
export default function EditPatientPlanPage() {
  const params = useParams<{ patientId: string; planId: string }>();
    ...
    async function load() {
      ...
      const loadedItems: PlanItem[] = planItems
        ...
        .map((pi) => ({
          ...
          exerciseName:
            (pi.exercises as unknown as { id: string; name: string } | null)
              ?.name ?? "",
  ...
  const basePath = `/physio/patients/${params.patientId}/plans`;
  ...
  if (!patientId || !initialPlan) {
    return (
      <div>
        <Header title="Modifica scheda" backHref={basePath} />
